    ...
    module Commands
      class Interactive < Dry::CLI::Command
        ...
        SERIAL_QUESTIONS = {
          ...
          mem: 'How much memory would you like to request? (MB)',
        ...
        ARRAY_QUESTIONS = {
          ...
          partition: 'Which partition would you like to use?',
          time: 'How long should each array task run for?',
          cpus_per_task: 'How many CPU cores should each array task use?',
          mem: 'How much memory should each array task use? (MB)',
          modules: 'What modules would you like to load?',
          command: 'What command should each array task run?'
        }.freeze
        ...
        MPI_QUESTIONS = {
          ...
          ntasks: 'How many MPI tasks would you like to run in total?',
          cpus_per_task: 'How many CPU cores should each MPI task use?',
          time: 'How long would you like your job to run for?',
          mem: 'How much memory would you like to request per node? (MB)',
          modules: 'What modules would you like to load?',
          command: 'What command would you like to run?'
        }.freeze
        ...
        ADVANCED_QUESTIONS = {
          ...
          error: 'What would you like to call the error output file generated by the script?',
          output: 'What would you like to call the output file generated by the script?'
        }
        ...
        QUESTION_BANK = {
          serial: SERIAL_QUESTIONS.merge(ADVANCED_QUESTIONS).freeze,

          mpi: MPI_QUESTIONS.merge(ADVANCED_QUESTIONS).freeze,

          gpu: GPU_QUESTIONS.merge(ADVANCED_QUESTIONS).freeze,

          array: ARRAY_QUESTIONS.merge(ADVANCED_QUESTIONS).freeze
        }
        ...
        def call(*)
          ...
          job_specific_questions.each do |key, question|
            ...
            when :command
              ...
              system('clear')
              puts
              puts pastel.bold('Congratulations!')
              puts
              puts 'You are now at a stage where you can submit your job to the queue.'
              puts 'There are also some more advanced options you can configure if you would like.'
              puts
              puts "These are:\n\n"

              ADVANCED_QUESTIONS.each_key do |key|
                case key
                when :track
                  puts pastel.bold.green('Job Tracking')
                  puts 'Adds helper functions to your script so you can check job progress later with alces-job status.'

                when :prepare
                  puts pastel.bold.magenta('Job Preparation')
                  puts 'Creates a dedicated working directory for this job so output and error files stay together.'

                when :output_file
                  puts pastel.bold.cyan('File Name')
                  puts 'Controls the name of the Slurm script file that will be written to disk.'

                when :error
                  puts pastel.bold.red('Error File')
                  puts 'Controls where errors and warnings from the job will be written.'
                  puts pastel.dim('(This is only used when job preparation is disabled; prepared jobs store error files inside the job directory.)')

                when :output
                  puts pastel.bold.yellow('Output File')
                  puts 'Controls where normal output from the job will be written.'
                  puts pastel.dim('(This is only used when job preparation is disabled; prepared jobs store output files inside the job directory.)')

                end

                puts
              end
              break unless prompt.yes?(pastel.bold('Would you like to configure these advanced options?'))
            when :prepare
            ...
            when :output_file
              file_name_question(key, question, flags, pastel, prompt)
            when :output
              output_file_question(key, question, flags, pastel, prompt)
            when :error
              error_file_question(key, question, flags, pastel, prompt)
            end
          end
          ...
          loop do
            ...
            when :partition
              ...
              if previous_partition != flags[:partition]
                ...
                if flags[:nodes] && flags[:nodes] > node_count
                  ...
                  flags[:nodes] = prompt.ask(pastel.bold.yellow(job_specific_questions[:nodes]), default: flags[:nodes]) do |q|
                    q.validate(/\A\d+\z/)
            ...
            when :output_file
              file_name_question(field, job_specific_questions[field], flags, pastel, prompt)
            when :output
              output_file_question(field, job_specific_questions[field], flags, pastel, prompt)
            when :error
              error_file_question(field, job_specific_questions[field], flags, pastel, prompt)
            end
          ...
          unless valid_manual_editing
            ...
            if prompt.yes?('Would you like to save these settings to a reusable profile?', default: false)
              ...
              puts
              puts "Profile saved to #{pastel.green(saved_profile_path)}"
            end
          end
          ...
          puts "\nScript has been saved to #{pastel.green(file_path)}"
          exit(0) unless prompt.yes?("\nWould you like to submit this job to SBATCH?", default: false)
        ...
        def prepare_question(key, question, flags, pastel, prompt)
          ...
          puts pastel.bold.magenta("\nJob Preparation\n")
          puts "This will create a #{pastel.bold('dedicated working directory')} for this job using the job name and job ID.\nThis step is #{pastel.underline('optional')}."
        ...
        def gres_question(key, question, flags, pastel, prompt, partition_info)
          ...
          puts pastel.bold.magenta("\nGPUs\n")
          puts "A GPU (Graphics Processing Unit) is a special processor used for highly parallel work, such as machine learning, simulations, and some scientific workloads.\n"
          ...
          flags[key] = prompt.ask(pastel.bold.magenta(question), default: flags[key] || DEFAULT_VALUES[key] || 1) do |q|
            q.validate do |input|
              input.to_s.match?(/\A\d+\z/) &&
        ...
        # Prompts the user for the file name settings for the script
        # @param [Symbol] key
        # @param [String] question
        # @param [Hash] flags
        # @param [Pastel::Delegator] pastel
        # @param [TTY::Prompt] prompt
        def file_name_question(key, question, flags, pastel, prompt)
          puts pastel.bold.cyan("\nFile Name\n")
          puts "What would you like to name your script? If left blank, it will default to the job name.\n\n"
          ...
          return unless flags[key].sub(/\.slurm$/, '') == flags[:job_name]
        ...
        def error_file_question(key, question, flags, pastel, prompt)
          ...
          puts pastel.bold.red("\nError File\n")
          puts "The error output file contains any errors or warnings produced by your script.\nWhat you would like to name the error output file? If left blank, it will default to #{flags[:job_name]}.err.\n\n"
          ...
          nil unless flags[key].sub(/\.err$/, '') == flags[:job_name]
        ...
        def output_file_question(key, question, flags, pastel, prompt)
          ...
          puts pastel.bold.yellow("\nOutput File\n")
          puts "The output file contains all output generated by your script.\n\n e.g. A script with the command #{pastel.yellow('echo')} 'Hello, world!', will produce an output file containing 'Hello, world'\n\nWhat would you like to name the output file? If left blank, it will default to #{flags[:job_name]}.out.\n\n"
          ...
          nil unless flags[key].sub(/\.out$/, '') == flags[:job_name]
