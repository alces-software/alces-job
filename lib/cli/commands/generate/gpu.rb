  ...
  module CLI
    module Commands
      class GPU < Templates::GenerateCommandTemplate
        ...
        desc 'Create a Slurm GPU job script'

        option :nodes, type: :integer, aliases: ['-N'], desc: 'Number of compute nodes'
        option :ntasks, type: :integer, aliases: ['-n'], desc: 'Number of tasks for the job'
        ...
        def call(**options)
          ...
          begin
            ...
            warn pastel.red('Failed to check existing script file:')
            warn pastel.red(e.message)
            exit(1)
          end
